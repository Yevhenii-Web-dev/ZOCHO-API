            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                @forelse($contacts as $contact)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="py-4 px-6 flex justify-between space-x-3">
                            <form method="POST" action="{{ route('contacts.destroy', $contact->id) }}">
                                @csrf
                                @method('DELETE')
                            <button type="submit" class="font-medium text-blue-600 dark:text-red-500 hover:underline">Delete</button>
                            </form>
                            <a href="{{ route('contacts.createDealInContact', $contact->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Add Deal</a>
                        </td>
                    </tr>

                @empty
                    <div class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-red-800">Contacts list is empty, try add new contact</div>
                @endforelse
                </tbody>
            </table>
        </div>
